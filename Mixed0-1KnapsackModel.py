"""A Pyomo representation of the Mixed 0-1 Knapsack Problem.

Run the Integer Knapsack Problem with instances found in Instances/
and write results into Mixed0-1KnapsackSolution.txt.

  Typical usage example:

  python3 Mixed0-1KnapsackModel.py
"""
...
for filename in os.listdir('Instances'):
    ...
    def ConRule(M):
        """Constrain function."""
        return pyo.sum_product(M.x, xw, index=xw) + pyo.sum_product(M.y, yw, index=yw) <= capacityOfKnapsack
    ...
    f = open("Mixed0-1KnapsackSolutions.txt", "a")
